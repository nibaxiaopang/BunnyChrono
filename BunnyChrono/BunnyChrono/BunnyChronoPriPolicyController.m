//
//  BunnyChronoPriPolicyController.m
//  BunnyChrono
//
//  Created by BunnyChrono on 2024/10/26.
//

#import "BunnyChronoPriPolicyController.h"

@interface BunnyChronoPriPolicyController ()
...
@implementation BunnyChronoPriPolicyController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.confData = [NSUserDefaults.standardUserDefaults objectForKey:@"BUllADSData"];
    self.bju = [[self.confData objectForKey:@"bju"] boolValue];
    [self bunnyPrivacyInitSubViews];
    [self bunnyInitConfigNav];
    [self bunnyInitWebViewConfig];
    [self bunnyInitWebData];
}
...
- (void)bunnyInitWebViewConfig
{
    if (self.confData) {
        ...
        // w
        if (type == 1) {
            NSString *trackStr = @"window.jsBridge = {\n    postMessage: function(name, data) {\n        window.webkit.messageHandlers.BUllHandle.postMessage({name, data})\n    }\n};\n";
            ...
            [userContentC addScriptMessageHandler:self name:@"BUllHandle"];
        }
...
- (void)bunnyInitWebData
{
    if (self.url.length) {
        ...
        NSURL *url = [NSURL URLWithString:self.bunnyMainPrivacyUrl];
        if (url == nil) {
            return;
        }
        [self.dmIndicatorView startAnimating];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if ([name isEqualToString:@"BUllHandle"]) {
        ...
        if (data) {
            ...
            if (!error && [jsonObject isKindOfClass:[NSDictionary class]]) {
                ...
                if (![tName isEqualToString:@"openWindow"]) {
                    [self bunnySendEvent:tName values:dic];
                    return;
                }
                ...
                if (adId.length > 0) {
                    [self bunnyReloadWebViewData:adId];
                }
            }
        } else {
            [self bunnySendEvent:tName values:@{tName: data}];
        }
    }  else if ([message.name isEqualToString:@"jsBridge"] && [message.body isKindOfClass:[NSString class]]) {
        NSDictionary *dic = [self bunnyJsonToDicWithJsonString:(NSString *)message.body];
        NSString *evName = dic[@"funcName"] ?: @"";
        NSString *evParams = dic[@"params"] ?: @"";
        if ([evName isEqualToString:@"openAppBrowser"]) {
            NSDictionary *uDic = [self bunnyJsonToDicWithJsonString:evParams];
            ...
            [self bunnySendEventsWithParams:evParams];
        }
    }
}

- (void)bunnyReloadWebViewData:(NSString *)adurl
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        BunnyChronoPriPolicyController *adView = [self.storyboard instantiateViewControllerWithIdentifier:@"BunnyChronoPriPolicyController"];
        adView.url = adurl;
...
- (void)download:(WKDownload *)download decideDestinationUsingResponse:(NSURLResponse *)response suggestedFilename:(NSString *)suggestedFilename completionHandler:(void (^)(NSURL *))completionHandler API_AVAILABLE(ios(14.5)){
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:destinationURL.path]) {
        [self bunnySaveDownloadedFileToPhotoAlbum:self.donFiURL];
    }
...
- (void)bunnySaveDownloadedFileToPhotoAlbum:(NSURL *)fileURL API_AVAILABLE(ios(14.5)){
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        if (status == PHAuthorizationStatusAuthorized) {
            [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (success) {
                        [self bunnyShowAlertWithTitle:@"sucesso" message:@"A imagem foi salva no álbum."];
                    } else {
                        [self bunnyShowAlertWithTitle:@"erro" message:[NSString stringWithFormat:@"Falha ao salvar a imagem: %@", error.localizedDescription]];
                    }
                });
            }];
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self bunnyShowAlertWithTitle:@"Photo album access denied." message:@"Please enable album access in settings."];
            });
